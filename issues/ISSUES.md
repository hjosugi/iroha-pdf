...
desktopは#066でin-place saveを持つが、mobileには無い。

**まず前提の訂正**: 当初「原本へ保存できない、Exportが別ファイルを作るだけ」と棚卸しに書いたが、それはExportを過小評価していた。emulatorで確認したところExportは正しく動く — 注釈をflattenし、page数・CJK・表テキストを保持した正当なPDFを生成し、OSの共有シートを開くのでDriveやFilesへ保存できる。焼き込み結果はpopplerでも検証済み。

つまりmobileに欠けているのは**開いたファイルそのものへの上書き**だけ。

現状で不可能な理由:

- `expo-document-picker`は`content://` URIを返すが、write権限をpersistしない
- `expo-file-system`の`File`は`file:///`前提で`content://`を扱えない
- 同packageのlegacy APIに`StorageAccessFramework`があるが、これは**directory**単位（`requestDirectoryPermissionsAsync` → `createFileAsync`）。選択した単一ファイルへの上書きには使えない

したがって#038のnative module（Android persistable URI permission / iOS security-scoped bookmark）が前提になる。

中間案として「ユーザーが一度フォルダを許可し、そこへ保存する」ならSAFのdirectory APIで実装可能。原本上書きではないが、アプリsandboxの外へ、再起動後も同じ場所へ保存できる。

## 075 [x] Multi-page PDFs report only one page on mobile
...
**修正:** `singlePage`を`enablePaging` + `horizontal`へ置き換えた。ライブラリのpropsを確認したところ、`enablePaging`は「1画面1ページ」という挙動を保ったままページ数を正しく報告する。#006のsingle-page viewerという前提を崩さずに済む。`onPageChanged`も第2引数で総数を渡すので、そちらでも更新する。

当初検討した「importでpdf-libを使い`pageCount`を保存する」案は採らなかった。importは現在`source.copy()`でnative copyしており、pdf-lib parseを挟むと300 MB scanでファイル全体をJSへ読むことになり#047と衝突するため。`WorkspaceDocument.pageCount`とDBの`page_count`列は依然として未使用のまま残っている。

emulatorで確認済み: 2ページのPDFが`1 / 2`と表示され、次ページボタンで`2 / 2`のAppendixへ移動でき、そこで次ページボタンが無効化される。
...
- 新設した`e2e-tauri`は`scripts/github/protect-main.sh`の必須checkに**入れていない**。CI上で一度もgreenになっていないjobを初日からmerge gateにはしない。実績がついたら昇格する
