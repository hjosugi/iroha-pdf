...
/**
 * Opens the practice document the first-run card offers (#65).
 *
 * It is served with the app (`public/sample/`, generated by
 * `release/sample/generate.mjs`), so it needs no dialog and no file scope, and it
 * is registered with no path: Save on it goes through Save as, so practising on
 * the sample can only ever write a file the user names.
 */
export function useOpenSample(): () => Promise<void> {
  const { provides } = useDocumentManagerCapability();

  return useCallback(async () => {
    if (!provides) return;
    const response = await fetch(new URL('sample/iroha-sample.pdf', document.baseURI));
    if (!response.ok) throw new Error(`The sample PDF could not be read (${response.status})`);
    const buffer = await response.arrayBuffer();
    const opened = await provides
      .openDocumentBuffer({ buffer, name: `${t('onboarding.sampleName')}.pdf` })
      .toPromise();
    registerOpenedFile(opened.documentId, null);
  }, [provides]);
}
