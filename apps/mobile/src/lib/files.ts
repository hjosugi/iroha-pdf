...
/**
 * The practice document the first-run card offers (#65), generated by
 * `release/sample/generate.mjs` and bundled with the app. It goes into the
 * library through the same import every other PDF takes, so it is an ordinary
 * document from then on: annotated, exported and deleted like any other.
 */
const SAMPLE_PDF = require('../../assets/sample/iroha-sample.pdf');

export async function importSamplePdf(title: string): Promise<WorkspaceDocument> {
  const asset = Asset.fromModule(SAMPLE_PDF);
  await asset.downloadAsync();
  const uri = asset.localUri ?? asset.uri;
  if (!uri) throw new Error('The sample PDF bundled with the app has no local file');
  return importPdfFile(new File(uri), `${title}.pdf`, 'local');
}
