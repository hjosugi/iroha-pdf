...
try {
  ...
  assert.match(workflow, /adb logcat -c[\s\S]*IrohaStylusEvidence:I[\s\S]*am force-stop app\.irohapdf\.mobile[\s\S]*iroha-pdf:\/\/\/viewer\/device-evidence-large-pdf[\s\S]*for _ in \$\(seq 1 120\)[\s\S]*uiautomator dump \/data\/local\/tmp\/iroha-stylus-window\.xml[\s\S]*pressure_tree_ready=true[\s\S]*Pen annotation tool[\s\S]*adb shell input tap[\s\S]*pen_tree_ready=true[\s\S]*stylus-reload\.png[\s\S]*instrumentation_status == 0[\s\S]*Verified persisted stylus pressure[\s\S]*Verified visible pressure accessibility state[\s\S]*stylus-pressure\.png stylus-reload\.png[\s\S]*89504e470d0a1a0a[\s\S]*pen_tree_ready[\s\S]*grep -q '<hierarchy'[\s\S]*package="app\.irohapdf\.mobile"[\s\S]*Ink width 2\\\.4\|ペンの太さ 2\\\.4[\s\S]*Pressure enabled\|筆圧を反映中/, 'stylus success must prove persistence, select Pen from the cold-reloaded target tree, and retain valid live and reload PNGs with a visible restored pressure badge');
  ...
  assert.match(stylusHarness, /dumpWindowHierarchy\(hierarchy\)[\s\S]*package=[^\n]*app\.irohapdf\.mobile[\s\S]*Pressure enabled[\s\S]*筆圧を反映中[\s\S]*instrumentation runner exits[\s\S]*screencap -p \/data\/local\/tmp\/iroha-stylus-pressure\.png/, 'instrumentation must prove the live foreground app and pressure state before capturing its PNG; the workflow owns the post-run, cold-reloaded XML');
