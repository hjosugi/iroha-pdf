...
try {
  ...
  assert.match(workflow, /adb logcat -c[\s\S]*IrohaStylusEvidence:I[\s\S]*am force-stop app\.irohapdf\.mobile[\s\S]*iroha-pdf:\/\/\/viewer\/device-evidence-large-pdf[\s\S]*for _ in \$\(seq 1 120\)[\s\S]*uiautomator dump \/data\/local\/tmp\/iroha-stylus-window\.xml[\s\S]*pressure_tree_ready=true[\s\S]*screencap -p[\s\S]*instrumentation_status == 0[\s\S]*Verified persisted stylus pressure[\s\S]*Verified visible pressure accessibility state[\s\S]*89504e470d0a1a0a[\s\S]*pressure_tree_ready[\s\S]*grep -q '<hierarchy'[\s\S]*package="app\.irohapdf\.mobile"[\s\S]*Pressure enabled\|筆圧を反映中/, 'stylus failures must retain evidence, and successful runs must prove persistence, a cold-reloaded pressure state, PNG integrity, and the target-app tree');
