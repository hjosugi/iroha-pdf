...
| page thumbnails | a 500-page document gets 500 slots and single digits of renders; scrolling the whole strip does not turn laziness off; a thumbnail decodes to a portrait picture of the page it names; leaving the tab releases every bitmap. Read from the DOM — a slot exists per page whether or not it has been drawn, so the pages held are the slots carrying an `<img>`. The byte budget itself is **not** exercised here: these fixture pages are nearly blank, so their thumbnails are a few KiB and 8 MiB is never approached. Eviction at the budget, in the right order, releasing the bytes and telling the strip, is covered by `src/thumbnails.test.ts` against sizes chosen to reach it. |
