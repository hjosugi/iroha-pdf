...
What a listing needs, and what this repository actually holds. Sizes are checked
by `release/branding/validate-assets.mjs`, so a missing or wrong-sized asset
fails CI rather than the Play Console.
...
| **Phone screenshots** | **both, mandatory** | **missing** |
| **iPad / tablet screenshots** | **App Store** (`supportsTablet` is true) | **missing** |

- [ ] Capture phone screenshots from a release-candidate build on a real device.
  Play needs at least two; the App Store needs a 6.9-inch set. These cannot be
  produced from a checkout — the desktop set is generated by Playwright from
  synthetic fixtures, and no equivalent exists for a device.
- [ ] Capture iPad screenshots (13-inch). Required because
  `apps/mobile/app.json` declares `ios.supportsTablet: true`; dropping tablet
  support would remove this requirement instead.
- [ ] Decide the listing text: title, short description, full description, in
  both Japanese and English. None is drafted anywhere in the repository.
